	...
	public class FlexibleGridLayout : LayoutGroup
	{
		...
		public override void CalculateLayoutInputHorizontal()
		{
			...
			if (fitType == FitType.Width || fitType == FitType.Height || fitType == FitType.Uniform)
			{
				fitX = true;
				fitY = true;
				float sqrRt = Mathf.Sqrt(transform.ChildCountActive());
			...
			{
				rows = Mathf.CeilToInt(transform.ChildCountActive() / (float) columns);
			}
			else if (fitType == FitType.Height || fitType == FitType.FixedRows)
			{
				columns = Mathf.CeilToInt(transform.ChildCountActive() / (float) rows);
			}
