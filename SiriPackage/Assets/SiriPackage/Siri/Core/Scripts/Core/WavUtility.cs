...
public class WavUtility
{
    ...
    private const string DirName = "Recordings";
    ...
    public static void CutSilence(ref AudioClip clip, float time)
    {
        //create list data
        ...
        //Cut index
        int i = (int)(time * clip.frequency);
        if (i > samples.Count)
            return;
        
        samples.RemoveRange(i, samples.Count - i);

        clip = AudioClip.Create("CutSilence", samples.Count, clip.channels, clip.frequency, false);
    ...
    public static AudioClip TrimSilence(List<float> samples, float min, int channels, int hz, bool _3D)
    {
        int i;
	...
	public static byte[] FromAudioClip (AudioClip audioClip, out string filepath, bool saveAsFile = true, string dirname = DirName)
	{
		...
		if (saveAsFile) {
			filepath = string.Format ("{0}/{1}/{2}.{3}", GetSavePath, dirname, DateTime.UtcNow.ToString ("yyMMdd-HHmmss-fff"), "wav");
